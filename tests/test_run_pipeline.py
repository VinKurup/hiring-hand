...
def test_run_stages_3_to_5_end_to_end(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(run_pipeline, "_load_resume", lambda pdf: object())
    monkeypatch.setattr(run_pipeline, "_load_resume_text", lambda resume: "RESUME TEXT")
    monkeypatch.setattr(run_pipeline, "_load_github_text", lambda resume: "GH TEXT")
    ...
    monkeypatch.setattr(
        run_pipeline, "match", lambda p, resume_text, github_text, model=None: report
    )
...
def test_cached_stage_is_reused(tmp_path, monkeypatch):
    ...
    cache = tmp_path / "cache"
    cache.mkdir()
    (cache / "role_profile.json").write_text(
        json.dumps(
            {
                "title": "Cached Role",
                "job_count": 1,
    ...
    report = MatchReport(
        matches=[
            MustHaveMatch(
                skill="Go", status="strong", evidence="ok", claim_without_evidence=False
            )
        ],
        visibility_score=50,
        evidence_score=50,
